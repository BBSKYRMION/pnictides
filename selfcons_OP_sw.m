function [fESEs,fESExs] = self_OP(xs,sw,N,g,bc,ky,sdw,soc)
mu=0;
...
%}
%
Es=diag(sort(diag(Es),'descend')); 
[c, ind]=sort(diag(Es),'descend'); 
Ud=Ud(:,ind);
%
...
fETOp(i)=(Fuidj(i,i+1)-Fuidj(i+1,i)+Fdiuj(i,i+1)-Fdiuj(i+1,i))/4;
end
